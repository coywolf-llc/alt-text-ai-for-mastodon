...
async function validateKey() {
  ...
  if (result.ok) {
    await browser.storage.local.set({ lastValidated: Date.now() });
...
async function clearKey() {
  $('apiKey').value = '';
  await browser.storage.local.remove(['apiKey', 'lastValidated']);
  setStatus($('keyStatus'), 'Key cleared.', 'warn');
}
...
async function savePricing() {
  ...
  await browser.storage.local.set({ pricing });
  setStatus($('pricingStatus'), 'Pricing saved.', 'ok');
}

async function resetPricing() {
  await browser.storage.local.set({ pricing: DEFAULT_PRICING, pricingAsOf: DEFAULT_PRICING_AS_OF });
...
async function addInstance() {
  ...
  list.push(domain);
  await browser.storage.local.set({ instances: list });
  await browser.runtime.sendMessage({ type: 'reconcile' });
  $('instanceInput').value = '';
  renderInstances(list);
  const addedMsg = `Added ${domain}. Reload its tab to see the button.`;
...
async function removeInstance(domain) {
  ...
  await browser.storage.local.set({ instances: list });
  try {
    await browser.permissions.remove({ origins: [`https://${domain}/*`] });
  } catch (e) {
    /* ignore */
  }
  await browser.runtime.sendMessage({ type: 'reconcile' });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  browser.storage.onChanged.addListener((changes, area) => {
    ...
    if (changes.sessionCost || changes.sessionCount) {
      browser.storage.local.get(['sessionCost', 'sessionCount']).then((d) =>
